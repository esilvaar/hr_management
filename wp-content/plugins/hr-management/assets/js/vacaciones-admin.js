(function(){
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // =====================================================
        function mostrarNotificacionExito(titulo, mensaje, detalles) { const notif = document.createElement('div'); notif.className = 'alert alert-success alert-dismissible fade show shadow-lg'; notif.setAttribute('role','alert'); notif.style.position = 'fixed'; notif.style.top = '20px'; notif.style.right = '20px'; notif.style.zIndex = '9999'; notif.style.minWidth = '400px'; let detalleHtml = ''; if (detalles && detalles.length > 0) { detalleHtml = '<ul class="mb-0 mt-2 small">'; detalles.forEach(det => { detalleHtml += `<li>${det.nombre}: ${det.total_empleados} total, ${det.personal_vigente} vigente</li>`; }); detalleHtml += '</ul>'; } notif.innerHTML = `<div class="d-flex gap-2 align-items-start"><span style="font-size: 1.5rem;">✅</span><div class="flex-grow-1"><strong>${titulo}</strong><p class="mb-0">${mensaje}</p>${detalleHtml}</div><button type="button" class="btn-close" data-bs-dismiss="alert"></button></div>`; document.body.appendChild(notif); setTimeout(() => { notif.remove(); }, 5000); }
        function mostrarNotificacionError(titulo, mensaje, errores) { const notif = document.createElement('div'); notif.className = 'alert alert-danger alert-dismissible fade show shadow-lg'; notif.setAttribute('role','alert'); notif.style.position = 'fixed'; notif.style.top = '20px'; notif.style.right = '20px'; notif.style.zIndex = '9999'; notif.style.minWidth = '400px'; let erroresHtml = ''; if (errores && errores.length > 0) { erroresHtml = '<ul class="mb-0 mt-2 small">'; errores.forEach(error => { erroresHtml += `<li>${error}</li>`; }); erroresHtml += '</ul>'; } notif.innerHTML = `<div class="d-flex gap-2 align-items-start"><span style="font-size: 1.5rem;">❌</span><div class="flex-grow-1"><strong>${titulo}</strong><p class="mb-0">${mensaje}</p>${erroresHtml}</div><button type="button" class="btn-close" data-bs-dismiss="alert"></button></div>`; document.body.appendChild(notif); setTimeout(() => { notif.remove(); }, 6000); }
